...
public class DijkstraGuided {
    ...
    public static void findShortestPath(WeightedNode source, WeightedNode destination)  {
        // En node og den node vi kom fra. Skal bruges til at printe vejen fra start til slut (backtracker)
        Map<WeightedNode, WeightedNode> prev = new HashMap<>();

        // Gemmer den bedste og billigste kendte dist til hver node
        ...
        queue.add(new NodeWithDist(source, 0)); // Starter fra 0
        dist.put(source, 0);
        ...
        while (!queue.isEmpty()) {
            ...
            NodeWithDist current = queue.poll(); // Gripper fat i næste ukendte node
